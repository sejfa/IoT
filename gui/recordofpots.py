...
class RecordOfPots(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.tabview = customtkinter.CTkTabview(self.frame_info, fg_color="#f6f6f6", border_color="#f6f6f6", bg_color="#f6f6f6", height=300, width=320)
        self.header = create_header(self.frame_info, "Records of Pots", 165, 10)
        self.tab1 = self.tabview.add("Add pot")
        self.tab2 = self.tabview.add("Update pot data")
        self.tab3 = self.tabview.add("Delete pot")
        self.tabview.set("Add pot")
            
        small_label(self.tab1, "Add pot", 10, 35)
        small_label(self.tab1, "Select the plant you want to place in pot", 10, 50)
    ...
    def place_widgets(self):
        ...
        self.delete_pot_menu.place(x=220 ,y=30)
        self.save_button.place(x=110, y=170)
        self.delete_button.place(x=110, y=170)
        #self.home_button.place(x=300, y=400)

    """def bind_widgets(self):
        self.save_button.configure(command=self.insert_pot_data)
        self.update_button.configure(command=self.update_pot_data)
    ...
    def insert_pot_data(self):
        ...
        try:
            self.c.execute("SELECT Plant FROM RecordsOfPOTS WHERE (Pot=?)",
            ...
            if self.result:
                ...
                self.c.execute("INSERT INTO RecordsOfPots (Pot) VALUES (?,?)",
                        (self.add_entry.get().capitalize()))
                self.updated_list()
                
                self.conn.commit()
                messagebox.showinfo("Success", "Pot is successfully added!")
                clear(self.add_entry)
    ...
    def insert_updated_data(self):
        ...
        try:
            ...
            if self.result:
                self.c.execute("UPDATE RecordsOfPots set Plant=? WHERE Pot=?",
