...
class LivingRoomHosta(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.header = create_label(self.hosta_info, "Living room - small table near the couch - Hosta", 15, 5)
    ...
    def place_widgets(self):
        self.hosta_info.place(x=2, y=2, width=483, height=215,)
    ...
    def display_data(self):
        ...
        if not self.displayed:
            self.h= sensor_hum()
            self.hum = create_smaller_label(self.hosta_info, self.h, 10, 45)
            self.t = sensor_temp()
            self.temp = create_smaller_label(self.hosta_info, self.t, 10, 80)
            self.b = sensor_bright()
            self.bright = create_smaller_label(self.hosta_info, self.b, 10, 115)
            self.p = sensor_ph()
            self.ph = create_smaller_label(self.hosta_info, self.p, 10, 150)
            self.s = sensor_sal()
            self.sal = create_smaller_label(self.hosta_info, self.s, 10, 185)
            self.displayed = True
    ...
    """def display_optimal_data(self):
        ...
        self.ph.destroy()
        self.sal.destroy()
        self.hosta_info.update()

        self.opt_hum = get_optimal_hum()
        self.optimal_hum = create_smaller_label(self.hosta_info, self.opt_hum, 15, 45)
        self.opt_temp = get_optimal_temp()
        self.optimal_temp = create_smaller_label(self.hosta_info, self.opt_temp, 15, 80)
        self.opt_bright = get_optimal_bright()
        self.optimal_bright = create_smaller_label(self.hosta_info, self.opt_bright, 15, 115)
        self.opt_ph = get_optmal_ph()
        self.optimal_ph = create_smaller_label(self.hosta_info, self.opt_ph, 15, 150)
        self.opt_sal = get_optimal_sal()
        self.optimal_sal = create_smaller_label(self.hosta_info, self.opt_sal, 15, 185)
"""
