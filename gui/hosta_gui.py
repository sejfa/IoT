...
from utils.util import get_image, create_label, create_smaller_label, get_background
...
class HostaPage(tk.Frame):
    ...
    def create_widgets(self):
        self.hosta_info = tk.LabelFrame(self, background=get_background())
    ...
    def place_widgets(self):
        self.hosta_info.place(x=80, y=120, width=313, height=215,)
    ...
    def display_data(self):
        ...
        if not self.displayed:
            self.h= plant_hum()
            self.hum = create_smaller_label(self.hosta_info, self.h, 5, 15)
            self.t = plant_temp()
            self.temp = create_smaller_label(self.hosta_info, self.t, 5, 55)
            self.b = plant_bright()
            self.bright = create_smaller_label(self.hosta_info, self.b, 5, 95)
            self.p = plant_ph()
            self.ph = create_smaller_label(self.hosta_info, self.p, 5, 135)
            self.s = plant_sal()
            self.sal = create_smaller_label(self.hosta_info, self.s, 5, 175)
            self.displayed = True
    ...
    """ def display_optimal_data(self):
        ...
        self.ph.destroy()
        self.sal.destroy()
        self.hosta_info_info.update()

        self.opt_hum = get_optimal_hum()
        self.optimal_hum = create_smaller_label(self.hosta_info_info, self.opt_hum, 15, 45)
        self.opt_temp = get_optimal_temp()
        self.optimal_temp = create_smaller_label(self.hosta_info_info, self.opt_temp, 15, 80)
        self.opt_bright = get_optimal_bright()
        self.optimal_bright = create_smaller_label(self.hosta_info_info, self.opt_bright, 15, 115)
        self.opt_ph = get_optmal_ph()
        self.optimal_ph = create_smaller_label(self.hosta_info_info, self.opt_ph, 15, 150)
        self.opt_sal = get_optimal_sal()
        self.optimal_sal = create_smaller_label(self.hosta_info_info, self.opt_sal, 15, 185)
        """
